...
class Category(models.Model):
	...
	def get_absolute_url(self):
		return reverse('sharing:category', kwargs={'pk': self.pk})
...
class Tag(models.Model):
	...
	def get_absolute_url(self):
		return reverse('sharing:tag_list', kwargs={'pk': self.pk})
...
class Post(models.Model):
	...
	tags = models.ManyToManyField(Tag, verbose_name='标签类型', blank=True)

	# 文章作者，这里 User 是从 django.contrib.auth.models 导入的。
	# django.contrib.auth 是 Django 内置的应用，专门用于处理网站用户的注册、登录等流程，User 是 Django 为我们已经写好的用户模型。
	# 这里我们通过 ForeignKey 把文章和 User 关联了起来。
	# 因为我们规定一篇文章只能有一个作者，而一个作者可能会写多篇文章，因此这是一对多的关联关系，和 Category 类似。
	author = models.ForeignKey('login.User', verbose_name='作者', on_delete=models.CASCADE)
	...
	def get_absolute_url(self):
		return reverse('sharing:post', kwargs={'pk': self.pk})
